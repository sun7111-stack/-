...
class EmissionFactorItem(Base):
    ...
    year = Column(Integer, nullable=True, index=True, comment="因子年份")
    version_tag = Column(String(50), default="", index=True, comment="版本标签")
    valid_from = Column(String(20), default="", comment="生效起始日期")
    valid_to = Column(String(20), default="", comment="生效截止日期")
    method = Column(String(50), default="default", index=True, comment="核算方法版本")
    source_type = Column(String(50), default="government_factor", index=True, comment="因子来源类型")
    dq_factor_level = Column(String(10), default="C", comment="因子数据质量等级")
    gsd_factor = Column(Float, default=0.1, comment="因子相对不确定性参数")
